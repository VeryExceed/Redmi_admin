<template>
	<div class="bg-white px-3" style="margin: -20px;margin-top: -1rem; margin-bottom: 0!important;">
		...
		<!-- 新增/修改模态框 -->
		<el-dialog width="80%" title="添加类型" :visible.sync="createModel" top="10vh">
			<!-- 表单内容 -->
			<el-form :rules="rules" ref="form" :model="form" label-width="80px">
				...
				<el-form-item label="关联规格">
					<div class="d-flex">
						<span class="sku-list-item px-3 py-2 border mr-2 rounded" style=" line-height: initial;"
					...
					<el-table :data="value_list" style="width: 100%">
						...
						<el-table-column label="属性值">
							<template slot-scope="scope" v-if="scope.row.type !== 'input'">
								<el-input type="textarea" v-if="scope.row.isedit" size="mini" v-model="scope.row.default"
									placeholder="一行为一个属性值，多个属性值用换行输入"></el-input>
								<span v-else>{{scope.row.default}}</span>
							</template>
						</el-table-column>
						<el-table-column label="操作" width="180">
							<template slot-scope="scope">
								<el-button @click="editRow(scope)" v-if="scope.row.type !== 'input'" type="text"
									size="mini">{{scope.row.isedit? '完成':'编辑属性值'}}</el-button>
			...
			submit() {
				this.$refs.form.validate(res => {
					...
					this.value_list.forEach((item, index) => {
						...
						if (item.order == '') {
							result = result && false
							message.push('第' + no + '排序不能为空')
						}
						if (item.name == '') {
							result = result && false
							message.push('第' + no + '属性名称不能为空')
						}
						if (item.type !== 'input' && item.default == '') {
							result = result && false
							message.push('第' + no + '属性值不能为空')
						}
					})
					if (!result) {
						var temp = '';
